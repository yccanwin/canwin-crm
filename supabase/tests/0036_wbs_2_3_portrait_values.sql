...
select is((select allowed_filter_operators from public.portrait_field_definitions where id=23501),array['is_set','is_not_set','contains'],'searchable text operators fixed');
select is((select allowed_filter_operators from public.portrait_field_definitions where id=23502),array['is_any_of','is_none_of','is_unknown'],'single-select operators fixed');
select is((select allowed_filter_operators from public.portrait_field_definitions where id=23503),array['contains_any','contains_all','is_unknown'],'multi-select operators fixed');
select is((select allowed_filter_operators from public.portrait_field_definitions where id=23504),array['is_true','is_false','is_unknown'],'boolean operators fixed');
select is((select allowed_filter_operators from public.portrait_field_definitions where id=23505),array['equals','greater_than','greater_or_equal','less_than','less_or_equal','is_unknown'],'number operators fixed');

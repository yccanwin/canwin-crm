...
requireMatch(scale, /function\s+runPsqlFullText\([\s\S]*?result\.stdout\.trim\(\)\.length\s*===\s*0[\s\S]*?return\s+result\.stdout\.trim\(\)/, 'EXPLAIN must use a dedicated full-text psql reader instead of the scalar last-line reader.')
requireMatch(scaleIndexProbe, /const\s+planText\s*=\s*runPsqlFullText\s*\(/, 'The JSON EXPLAIN plan must be captured with the full-text reader.')
requireMatch(scale, /phase:'EXPLAIN_CAPTURED'[\s\S]*?plan_bytes:Buffer\.byteLength\(planText,'utf8'\)[\s\S]*?plan_lines:planText\.split\(\/\\r\?\\n\/\)\.length[\s\S]*?plan_sha256:createHash\('sha256'\)\.update\(planText\)\.digest\('hex'\)/, 'Scale diagnostics must bind the complete EXPLAIN capture using safe size, line-count, and SHA-256 metadata.')
requireMatch(scale, /Object\.keys\(safePlanCapture\)\.sort\(\)\.join\('\|'\)\s*!==\s*'eligible_count\|phase\|plan_bytes\|plan_lines\|plan_sha256'/, 'Scale plan-capture diagnostics must enforce the exact safe five-key shape.')
for (const phase of ['ELIGIBILITY_QUERY','ELIGIBILITY_COUNTED','EXPLAIN_QUERY','EXPLAIN_CAPTURED','PLAN_PARSED','PLAN_INSPECTED']) requireMatch(scale, new RegExp(`['"]${phase}['"]`), `Scale diagnostics must expose safe phase ${phase}.`)
...
forbidMatch(scale, /plan_capture\s*:\s*(?:planText|plan|evidenceFragments|migrationText|sourceBinding)/, 'Scale failure capture metadata must never expose a raw plan, SQL, migration, source binding, or evidence buffer.')
const scaleConsoleTokens = [...scale.matchAll(/console\.(?:log|error)\s*\(/g)]
if (scaleConsoleTokens.length !== 2) failures.push('Scale verifier must expose exactly one success console call and one failure console call.')
if ([...scale.matchAll(/console\.log\s*\(/g)].length !== 1 || !/console\.log\(output\)/.test(scale)) failures.push('Scale success output must use one exact console.log(output) call.')
if ([...scale.matchAll(/console\.error\s*\(/g)].length !== 1 || !/console\.error\(JSON\.stringify\(safeFailureOutput\)\)/.test(scale)) failures.push('Scale failure output must use one exact safe console.error call.')
forbidMatch(scale, /console\.(?:log|error)\s*\(\s*(?:planText|plan|evidenceFragments|migrationText|sourceBinding)\b/, 'Scale console output must never directly reference a raw plan, migration, source binding, or evidence buffer.')
const scaleFailurePath = scale.slice(scale.indexOf('main().catch'))
requireMatch(scaleFailurePath, /Object\.keys\(safePlanCapture\)\.sort\(\)\.join\('\|'\)[\s\S]*?!==\s*'eligible_count\|phase\|plan_bytes\|plan_lines\|plan_sha256'[\s\S]*?safePlanCapture\s*=\s*\{\s*phase:'WITHHELD_BY_SCANNER'/, 'Every early failure path must enforce the exact five-key capture shape before output.')
requireMatch(scale, /let\s+safeCanaries\s*=\s*\[\][\s\S]*?safeCanaries\s*=\s*canaries/, 'Scale failure diagnostics must retain the current synthetic canary set for catch-path scanning.')
requireMatch(scaleFailurePath, /const\s+captureScan\s*=\s*scanEvidence\(\[JSON\.stringify\(safePlanCapture\)\],\s*safeCanaries\)[\s\S]*?captureScan\.secret_pattern_counts[\s\S]*?captureScan\.pii_pattern_count[\s\S]*?captureScan\.document_storage_canary_hits[\s\S]*?captureScan\.forbidden_portrait_key_hits[\s\S]*?safePlanCapture\s*=\s*\{\s*phase:'WITHHELD_BY_SCANNER'/, 'Scale failure output must rescan every capture category with the real run canaries and fail closed before output.')
requireMatch(scaleFailurePath, /let\s+safeFailureOutput\s*=\s*\{\s*status:'FAIL',\s*stage:safeStage,\s*functional_errors:functionalErrors,\s*transport_errors:transportErrors,\s*intended_index_used:intendedIndexUsed,\s*disk_spill_count:diskSpillCount,\s*plan_capture:safePlanCapture,\s*plan_summary:safePlanSummary\s*\}/, 'Scale failure output must use the exact safe eight-key object literal.')
requireMatch(scaleFailurePath, /Object\.keys\(safeFailureOutput\)\.sort\(\)\.join\('\|'\)\s*!==\s*'disk_spill_count\|functional_errors\|intended_index_used\|plan_capture\|plan_summary\|stage\|status\|transport_errors'[\s\S]*?safeFailureOutput\s*=\s*\{\s*status:'FAIL',\s*stage:'PF23-OUTPUT_WITHHELD'/, 'Scale failure output must enforce the exact eight-key shape and use a fixed safe fallback.')
